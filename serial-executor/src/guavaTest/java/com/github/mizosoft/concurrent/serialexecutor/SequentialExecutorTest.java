...
public class SequentialExecutorTest extends TestCase {
  ...
  public void setUp() {
    ...
    e = new SerialExecutor(fakePool);
  }

  public void testConstructingWithNullExecutor_fails() {
    try {
      new SerialExecutor(null);
  ...
  public void testInterrupt_beforeRunRestoresInterruption() throws Exception {
    ...
    // returned by SerialExecutor. Clear the bit while checking so that the test doesn't hose
  ...
  public void testInterrupt_doesNotInterruptSubsequentTask() throws Exception {
    ...
    // Check that the interruption of a SerialExecutor's task is restored to the thread once
  ...
  public void testTaskThrowsError() throws Exception {
    ...
    try {
      final SerialExecutor executor = new SerialExecutor(service);
      Runnable errorTask =
          new Runnable() {
            @Override
            public void run() {
      ...
      // SerialExecutor by the time the barrier is satified
